 ...
 */
@interface JKPickBySlideRangeLeftElement : UIAccessibilityElement
@end

/**
 An accessibility element that increments and decrements the right slider
 */
@interface JKPickBySlideRangeRightElement : UIAccessibilityElement
@end

static const CGFloat kLabelsFontSize = 12.0f;

@implementation JKPickBySlideRange
...
- (void)initialiseControl {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        trackCircle.hidden = YES;
        
        [self.layer addSublayer:trackCircle];
        
        [self.trackCirclesArray addObject:trackCircle];
    }
    ...
    self.leftHandle.shadowColor = [UIColor colorWithRed:29/255.0 green:29/255.0 blue:29/255.0 alpha:0.2].CGColor;
    self.leftHandle.shadowOffset = CGSizeMake(0, 3);
    self.leftHandle.shadowOpacity = 1;
    self.leftHandle.shadowRadius = 4;
    ...
    self.rightHandle.shadowColor = [UIColor colorWithRed:29/255.0 green:29/255.0 blue:29/255.0 alpha:0.2].CGColor;
    self.rightHandle.shadowOffset = CGSizeMake(0, 3);
    self.rightHandle.shadowOpacity = 1;
    self.rightHandle.shadowRadius = 4;
...
- (void)layoutSubviews {
    ...
    if (self.enableStep) {
        for (int i = 0; i < self.trackCirclesArray.count; i++) {
            CAShapeLayer *trackCircle = self.trackCirclesArray[i];
            trackCircle.hidden = NO;
            
            trackCircle.bounds = CGRectMake(0, 0, 1, self.lineHeight);
            CGFloat stepWidth = self.sliderLine.frame.size.width / 4;
...
- (void)updateLabelValues {
    ...
    if (self.enableStep) {
        self.minLabel.string = [self.labels objectAtIndex:((int)self.selectedMinimum - 1)];
        self.maxLabel.string = [self.labels objectAtIndex:((int)self.selectedMaximum - 1)];
    } else {
...
{
  JKPickBySlideRangeLeftElement *element = [[JKPickBySlideRangeLeftElement alloc] initWithAccessibilityContainer:self];
...
{
  JKPickBySlideRangeRightElement *element = [[JKPickBySlideRangeRightElement alloc] initWithAccessibilityContainer:self];
  element.isAccessibilityElement = YES;
...
@implementation JKPickBySlideRangeLeftElement

- (void)accessibilityIncrement {
  JKPickBySlideRange* slider = (JKPickBySlideRange*)self.accessibilityContainer;
...
@implementation JKPickBySlideRangeRightElement
